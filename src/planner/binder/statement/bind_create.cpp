...
// Binder::Bind 处理CreateStatement
// 每个BoundStatement内部都有一个LogicalOperator
// Bind的过程就是给BoundStatement构建正确的LogicalOperator
BoundStatement Binder::Bind(CreateStatement &stmt) {
	BoundStatement result;
	// 因为每个SQL Statement 都会返回Result Set
	// Count -> affected rows
	result.names = {"Count"};
	result.types = {LogicalType::BIGINT};

	// in duckdb, different database objects like tables, views, indexs
	// CatalogType
	// CreateStatement->CreateInfo->type
	auto catalog_type = stmt.info->type;

	// CreateStatement可能有哪些
	// 1. create or replace
	...
	switch (catalog_type) {
	// duckdb 默认使用的是main schema
	// CREATE SCHEMA s1 语句
	// 在指定的schema中创建一个table
	// CREATEA TABLE s1.t (id INTEGER PRIMARY KEY);
	...
	// View in duckdb is a virtual table
	// View 的定义is stored by query that you can reference
	// it does not store data itself
	// purpose: simplify complex queries, read-only, no phsical-storage, dynamic results
	// Materialized View
	// 将结果存储起来就是Materialised View
	// DuckDB 不支持
	...
	// SequenceEntry 在数据库领域表示一个数据库对象用于生成一个序列
	// CREATE SEQUENCE serial;
	// CREATE SEQUENCE serial START 100;
	// CREATE SEQUENCE serial START 100 INCREMENT BY 2 MAXVALUE 1000;
	// LogicalCreate
	...
	// Table Macro
	// Powerful feature for creating reusable, parameterized table-generating functions

	// CREATE MACRO get_sales(start_date DATE, end_date DATE) AS
	// 	SELECT
	//		product_id,
	...
	// SELECT * FROM get_sales("2023-01-01", "2024-01-01")
	// 创建LogicalOpeartorType::LOGICAL_CREATE_MACRO
	case CatalogType::TABLE_MACRO_ENTRY: {
		...
		// MACRO_ENTRY
		// 一般是scalar macro
		// scalar macro 返回的是一个值，而不是一个table
		// CREATE TABLE fahrenheit_to_celsius(f) AS ((f - 32) * 5.0 / 9.0);
		// SELECT fahrenheit_to_celsius(77);
	...
	// 创建Index
	// 需要确保index在valid的table上
	// CREATE_UNIQUE_INDEX films_id_idx ON files (id);
	case CatalogType::INDEX_ENTRY: {
		...
		// duckdb中的base table是指
		// phsical table, exists in the database and stores actual data
		// 不是base table的
		// - views
		// - temporary tables
		// - derived tables
