...
unique_ptr<LogicalOperator> Binder::CreatePlan(BoundSelectNode &statement) {
	...
	// DuckDB 支持sample clause
	// SELECT *
	// FROM address
	// USING SAMPLE 1%;
	// 如果有SAMPLE语句的话，我们需要创建一个LogicalSample Operator
	// SELECT * FROM users USING SAMPLE 1%; 对应的Plan
	// PROJECTION
	// SAMPLE
	// SEQ_SCAN
	...
	// 处理Where Clause, 会生成Filter operator
	// SELECT * FROM users USING SAMPLE 1%; 对应的Plan
	// PROJECT
	// FILTER
	// SAMPLE
	...
	if (!statement.aggregates.empty() || !statement.groups.group_expressions.empty()) {
		// 处理Group By 语句
		// 会变成Aggregate Operator
		// 在SQL中, GROUP BY 比 Aggregate要优先
		/*
		┌─────────────┴─────────────┐
		│         AGGREGATE         │
		│   ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─   │
	...
	// Having: 总是和group by 配合使用
	// SELECT department, AVG(salary) FROM employees GROUP BY department
	// HAVING AVG(salary) > 5000
	// 创建一个LogicalFilter Operator
	if (statement.having) {
	...
	// The QUALIFY clause is a feature in some SQL dialects, including DuckDB, that allows filtering of results based on
	// window functions
