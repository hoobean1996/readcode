...
// Planner::CreatePlan本身不直接创建plan，而且coordinate process

// auto planner = Planner::Planner(ClientContext);
// planner->CreatePlan();
...
// CreatePlan 利用SQLStatement创建真正的Logical Plan
...
void Planner::VerifyPlan(ClientContext &context, unique_ptr<LogicalOperator> &op,
	...
	auto &config = DBConfig::GetConfig(context);
// 如果定义了DUCKDB_ALTERNATIVE_VERIFY
// 需要进行Serialization compatibility
