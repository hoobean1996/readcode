...
// BoundStatement Binder::Bind方法会基于SQLStatement的类型的不同来进行bind
// 其实就是做一次强制类型转换
// 但是由于bind_select会delegate到Bind(QueryNode)的情况
// Binder::Bind根据statement来进行bind

// SQL -> SQL Statement -> Binder::Bind(SQLStatement) -> Binder::Bind(SelectStatement)
// -> Create QueryNode -> Binder::Bind(QueryNode)
...
// Binder::Bind 有几个重载的方法
// bind_select 会delegate到这里
/*
    SELECT_NODE的情况:
    SELECT employees.name, departments.dept_name
    FROM employees
    JOIN departments ON employees.dept_id = departments.id
    WHERE employeers.salary > 50000;
        ...
        WITH RECURSIVE employee_hierarchy AS (
            ...
            UNION ALL
                ...
                SELECT * FROM employee_hierearchy;
*/
