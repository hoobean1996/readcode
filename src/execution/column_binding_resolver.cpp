...
// ColumnBindingResovler::VisitOperator 遍历LogicalOperator
void ColumnBindingResolver::VisitOperator(LogicalOperator &op) {
	switch (op.type) {
		// This is a more specialized join operation, often used in time-series data or when dealing with data that has
		// temporal aspects.
		// 常用于时间序列数据的join
		/*
		SELECT * FROM stock_prices p
ASOF JOIN company_events e ON p.time <= e.time AND p.company_id = e.company_id

		*/
	case LogicalOperatorType::LOGICAL_ASOF_JOIN:
	// LOGICLA_COMPARISION_JOIN: 标准的join
