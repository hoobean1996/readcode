...
DatabaseHeader DatabaseHeader::Read(ReadStream &source) {
  ...
  if (header.vector_size != STANDARD_VECTOR_SIZE) {
    throw IOException("Cannot read database file: DuckDB's compiled vector "
                      "size is %llu bytes, but the file has a "
...
void SingleFileBlockManager::CreateNewDatabase() {
  auto flags = GetFileFlags(true /*create_new*/);
  ...
  // SerializeHeaderStructure write header to file
...
void SingleFileBlockManager::LoadExistingDatabase() {
  ...
  if (!handle) {
    ...
    throw IOException("Cannot open database \"%s\" in read-only mode: database "
                      "does not exist",
                      path);
  }
...
void SingleFileBlockManager::ReadAndChecksum(FileBuffer &block,
  ...
  if (stored_checksum != computed_checksum) {
    throw IOException("Corrupt database file: computed checksum %llu does not "
                      "match stored checksum %llu in block "
                      "at location %llu",
...
void SingleFileBlockManager::Initialize(const DatabaseHeader &header,
  ...
  if (block_alloc_size.IsValid() &&
    ...
    throw InvalidInputException("cannot initialize the same database with a "
...
unique_ptr<Block>
SingleFileBlockManager::ConvertBlock(block_id_t block_id,
...
unique_ptr<Block>
SingleFileBlockManager::CreateBlock(block_id_t block_id,
                                    FileBuffer *source_buffer) {
...
void SingleFileBlockManager::ReadBlocks(FileBuffer &buffer,
  ...
  for (idx_t i = 0; i < block_count; i++) {
    ...
    // verify the checksum
    if (stored_checksum != computed_checksum) {
      throw IOException("Corrupt database file: computed checksum %llu does "
                        "not match stored checksum %llu in block "
                        "at location %llu",
...
void SingleFileBlockManager::Write(FileBuffer &buffer, block_id_t block_id) {
  D_ASSERT(block_id >= 0);
  ChecksumAndWrite(buffer, BLOCK_START + NumericCast<idx_t>(block_id) *
                                             GetBlockAllocSize());
}
...
public:
  FreeListBlockWriter(MetadataManager &manager,
      ...
      : MetadataWriter(manager),
        free_list_blocks(std::move(free_list_blocks_p)), index(0) {}
...
void SingleFileBlockManager::WriteHeader(DatabaseHeader header) {
  ...
  if (config.options.checkpoint_abort ==
    ...
    throw FatalException("Checkpoint aborted after free list write because of "
                         "PRAGMA checkpoint_abort flag");
  }

  if (!options.use_direct_io) {
    // if we are not using Direct IO we need to fsync BEFORE we write the header
    // to ensure that all the previous blocks are written as well
    handle->Sync();
  }
...
void SingleFileBlockManager::TrimFreeBlocks() {
  if (DBConfig::Get(db).options.trim_free_blocks) {
    for (auto itr = newly_freed_list.begin(); itr != newly_freed_list.end();
      ...
      --itr;
      // Trim the range.
      handle->Trim(BLOCK_START +
                       (NumericCast<idx_t>(first) * GetBlockAllocSize()),
